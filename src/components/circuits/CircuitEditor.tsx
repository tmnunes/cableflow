        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="text-sm">
            ...
            >
              ...
              <SelectContent>
                {(activeRuleSet?.installationMethods ?? [{ id: 'unspecified', name: t('electricalCircuits.unspecifiedMethod') }]).map(
                  (method) => (
          ...
          <label className="text-sm">
            ...
            <Select
              value={circuit.selectedProtectionMaterialId ?? 'none'}
              onValueChange={(value) =>
                patch({ selectedProtectionMaterialId: value === 'none' ? undefined : value })
              }
            >
              <SelectTrigger>
        ...
        <section className="rounded-lg border border-border/70 bg-muted/20 p-4">
          ...
          <dl className="grid gap-2 sm:grid-cols-2">
            <div className="flex justify-between gap-3 text-sm">
              ...
              <dd className="font-mono">{formatNumber(design?.installedPower, locale)} {t('electrical.watts')}</dd>
            </div>
            <div className="flex justify-between gap-3 text-sm">
              <dt>{t('electricalCircuits.designPower')}</dt>
              <dd className="font-mono">{formatNumber(design?.designPower, locale)} {t('electrical.watts')}</dd>
            </div>
            <div className="flex justify-between gap-3 text-sm">
              <dt>{t('electricalCircuits.designCurrent')}</dt>
              <dd className="font-mono">{formatNumber(design?.designCurrent, locale)} {t('electrical.amperes')}</dd>
            </div>
            <div className="flex justify-between gap-3 text-sm">
            ...
            <div className="flex justify-between gap-3 text-sm">
              ...
              <dd>{design?.protection?.option ? protectionLabel(design.protection.option) : t('electricalCircuits.none')}</dd>
            </div>
            <div className="flex justify-between gap-3 text-sm">
            ...
            <div className="flex justify-between gap-3 text-sm">
              ...
              <dd>{design?.protection?.validated ? t('electricalCircuits.validated') : t('electricalCircuits.notValidated')}</dd>
            </div>
              ...
              {(design?.additionalProtections ?? [])
                ...
                .map((item) => (
                  <li key={`${item.type}-${item.reason ?? ''}`}>
                    {t('electricalCircuits.additionalProtections')}: {protectionLabel(item.recommended ?? { type: item.type })}
